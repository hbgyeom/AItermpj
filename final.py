...
# Load model
model_path = r"C:\Users\HBG\codes\project\detection\detection_model.h5"
...
def transcribe_audio():
    ...
    while True:
        ...
        if audio is None:
            ...
            break
        try:
            raw_pcm = np.frombuffer(audio.frame_data, dtype=np.int16)
            normalized_audio = raw_pcm / 32768.0
            sample_rate = audio.sample_rate
            max_samples = 5 * sample_rate
            trimmed_audio = normalized_audio[:max_samples]
            mfcc = librosa.feature.mfcc(y=trimmed_audio,
                                        sr=sample_rate, n_mfcc=13)
            prediction = detection_model.predict(mfcc)
